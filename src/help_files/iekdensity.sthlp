...
{pstd}{it:Required options:}{p_end}
	{synopt :{opt treat:var(varname)}}Treatment (dummy or factor) variable.{p_end}
...
{title:Options}
	...
	{pstd}{it:{ul:{hi:Content options:}}}{p_end}
		...
		{phang}{cmdab:control(}{it:{help numlist:numlist}}{cmd:)} indicates which value the variable {opt treat:var(varname)} takes for the control group. This is usually equal to 0 when the treatment is binary, but may vary when dealing with multiple treatment arms.{p_end}
...
{title:Examples}
	...
	{pstd} {inp:iekdensity} {it:auto} , {inp:treatvar(}{it:treatment}{inp:)}
	...
	{pstd} {inp:iekdensity} {it:auto} , {inp:treatvar(}{it:treatment}{inp:)} {inp:stat(}{it:p50}{inp:)}
	...
	{pstd} This is an easy way to add descriptive information to the graph. This will output the same graph as above with the addition of two vertical lines for the medians of the control and treatment groups.
		...
		{pstd} {inp:iekdensity} {it:auto} , {inp:treatvar(}{it:treatment}{inp:)} {inp:stat(}{it:p50}{inp:)} {inp:statstyle(}lpattern(dash) lwithd(2){inp:)}
		...
		{pstd} {inp:iekdensity} {it:auto} , {inp:treatvar(}{it:treatment}{inp:)} {inp:stat(}{it:p50}{inp:)} {inp:statstyle(}lpattern(dash) lwithd(2){inp:)} {inp:color(}eltblue edkblue{inp:)}
	...
	{pstd} {inp:iekdensity} {it:auto} , {inp:treatvar(}{it:treatment}{inp:)} {inp:stat(}{it:p50}{inp:)} {inp:effect}
	...
	{pstd} This adds a note to the graph, displaying the treatment effect in terms of point estimate, standard error and statistical significance.
		...
		{pstd} {inp:iekdensity} {it:auto} , {inp:treatvar(}{it:treatment}{inp:)} {inp:stat(}{it:p50}{inp:)} {inp:effect} {inp:effectformat(}%9.0fc{inp:)}
	...
	{pstd} {inp:iekdensity} {it:auto} , {inp:treatvar(}{it:treatment}{inp:)} {inp:effect} {inp:absorb(}{it:foreign}{inp:)}
	
	{pstd} The treatment effect is now derived from a regression controlling for the variable {it:foreign} fixed effects.
		
	{pstd} {hi:Example 4.2}
		
	{pstd} {inp:iekdensity} {it:auto} , {inp:treatvar(}{it:treatment}{inp:)} {inp:effect} {inp:absorb(}{it:foreign}{inp:)} {inp:regressionoptions(}{cluster(foreign)}{inp:)}
	...
	{pstd} {inp:iekdensity} {it:auto} , {inp:treatvar(}{it:treatment}{inp:)} {inp:kdensityoptions(}epan2 bwidth(5){inp:)} 
