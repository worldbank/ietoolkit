...
capture program drop iebaltab
	...
	// 1 Prepare dataset -----------------------------------------------------
		dataprep `varlist' `if' `in', grpvar(`grpvar')
	...
	restore	
		...
		foreach balancevar of varlist `balancevars' {

			
			* Balance tests
			foreach pair {
				reg `balancevar' `grpvar'
				
				* Conditional difference
				local diff_`ttest_pair' =  _b[`tempvar_thisGroupInPair']
				
				* Standard deviation
				local se_`ttest_pair' =  _se[`tempvar_thisGroupInPair']
