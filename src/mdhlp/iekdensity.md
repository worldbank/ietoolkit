...
__by__(treatmentvar) indicates which variable should be used to identify the treatment assignment. This can be a dummy variable (0/1) or a factor variable, when there are multiple treatments.

## Content options

__stat__(_string_) specifies a descriptive statistic to be plotted over the kernel density graph. In particular, vertical lines for each treatment group are added. Accepted statistics are: _mean, p1, p5, p50, p75, p90, p95, p99, min_ and _max_.
...
__control__(_numlist_) indicates which value the variable __by__(_treatmentvar_) takes for the control group. This is usually equal to 0 when the treatment is binary, but may vary when dealing with multiple arms.
...
__**reg**ressionoptions__(_string_) indicates other options to be employed for the treatment effect estimations, for example suppress constant term (__**nocons**tant__) or clustered standard errors (__**cl**uster__(_varname_)). All options for regress (or areg when option __absorb()__ is specified) are accepted.
...
This is the most basic way to run this command. This will output a graph with the distributions of the variable of interest (price in this case) by treatment assignment.
...
You can also see the code, make comments to the code, see the version history of the code, and submit additions or edits to the code through the [GitHub repository](https://github.com/worldbank/ietoolkit) for `ietoolkit`.
