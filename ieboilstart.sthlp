...
gathers best practices used across the Stata community to mitigate the risk of behaving
differently. See more details {help ieboilstart##comp:below}.
...
{pstd}{cmdab:ieboilstart} creates a dofile for a project that includes a sections of standardized
	...
	settings across users (for example, {help set varabbrev:varabbrev}), and
	some settings are meant to mitigate the risk that different versions of Stata
...
{pstd}A command like this is next to impossible to make comprehensive, so the ambition
	is mostly to provide an easy option to generate advance settings that are much
	better than having no boilerplate code at all. Suggestions for additions or improvements
...
{pstd}All of these settings are very common in the Stata community, and the default
	...
	on what settings user written commands can modify, and this is the reason that there
	are two parts to specifying this command and why it is important to use {cmd:include} when
	executing the dofile generated.

{marker comp}{...}
{title:Compatibility Across Different Stata:}

{pstd}The first part of {cmdab:ieboilstart} generates a dofile with the most
	compatibility practically possible between Stata 11.0-14.1, Stata Small/IC/SE/MP and Stata for
	PC/Mac/Linux. {it:Compatibility} does not mean an absolute guarantee that all code will always
	...
	dofile includes if-statements skipping settings for users with other Stata types
	that would otherwise have generated an error. If possible it instead offers an alternative settings that works for that Stata type.

{pstd}In addition to version control, {cmdab:ieboilstart} allows a project to 	standardize user preference settings such as {help set varabbrev:varabbrev} that 		can cause code to not be compatiable across users.

{pstd}The second part of {cmdab:ieboilstart} applies those settings. The first part
	does not set any settings. The second part can be repeated multiple times after
...
{pstd}Impact Evaluations and other research projects often span over several
	...
	results. See {help version:version} settings for more details. This method is included in {cmdab:ieboilstart}, however, {cmd:version} is intended for making code written in Stata 11 to work
...
{pstd}{hi:Warning 2:} Note that version settings has the same scope as a local. That means that
	...
	create a lot of work to a very small gain. If {cmdab:ieboilstart} is included in the master dofile,
	then it would be sufficient to use the command {help version:version} in individual dofiles
	with randomization instead of running the dofile generated by {cmdab:ieboilstart}.
...
{break}{cmdab:include} "`r(ieboil_dofile)'"
...
{pmore}{cmdab:include} "$dofile_folder/ieboil_settings.do"
