%%% Table created in Stata by command iebaltab
%%% (https://github.com/worldbank/ietoolkit)
%%% (https://dimewiki.worldbank.org/iebaltab)
%%% The command was specified exactly like this: 
%%% iebaltab weight , savecsv("run/iebaltab/outputs/iebt-csv24a") savexlsx("run/iebaltab/outputs/iebt-xlsx24a") texnotefile("run/iebaltab/outputs/iebt-tex24a-note") savetex("run/iebaltab/outputs/iebt-tex24a") replace groupvar(tmt_cl) nonote addnote(`"added a "tricky" note"')

\begin{tabular}{@{\extracolsep{5pt}}lcccccc}
\\[-1.8ex]\hline \hline \\[-1.8ex]
 & \multicolumn{2}{c}{(1)}  & \multicolumn{2}{c}{(2)}  & \multicolumn{2}{c}{(1)-(2)} \\
 & \multicolumn{2}{c}{0}  & \multicolumn{2}{c}{1}  & \multicolumn{2}{c}{Pairwise t-test}  \\
Variable & N & Mean/(SE) & N & Mean/(SE) & N & Mean difference \\ \hline \\[-1.8ex] 
weight   & 31    & 3084.516    & 43    & 2972.558    & 74    & 111.958   \\
 &   & (117.753)  &   & (130.959)  &   &  \\ [1ex]
\hline \hline \\[-1.8ex]

\end{tabular}
